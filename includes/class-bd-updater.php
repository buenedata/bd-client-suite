...
// Use unique class name to avoid conflicts with other BD plugins
if (!class_exists('BD_Client_Suite_Updater')) {
class BD_Client_Suite_Updater {
    private $plugin_file;
    ...
    public function __construct($plugin_file, $github_username, $github_repo) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("BD_Client_Suite_Updater initialized for {$this->plugin_basename} (slug: {$this->plugin_slug}, version: {$this->version})");
        }
    }
    ...
    public function plugin_info($result, $action, $args) {
        ...
        if (!is_wp_error($request) && wp_remote_retrieve_response_code($request) === 200) {
            ...
            $result = (object) [
                'name' => $data['name'] ?? 'BD Client Suite',
                'slug' => $this->plugin_slug,
                ...
                'short_description' => 'BD Client Suite - Professional WordPress branding and customization plugin',
                'sections' => [
                    'description' => wpautop(wp_kses_post($data['body'] ?? 'Professional WordPress branding and customization plugin that transforms the admin experience.')),
